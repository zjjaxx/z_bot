...
class Strategy(StrategyTemplate):
    ...
    def calc_boll_macd(self,data):
        ...
        daily_df['signal'] = 0
        daily_df.loc[buy_condition_bottom, 'signal'] = 2
    ...
    def exec_backtest(self,symbol):
        ...
        if not signal==0:
            ...
            #model 数据写入
            # 使用事务来确保所有操作的原子性
            self.save_strategy([symbol,signal,"boll_rsi_v2策略: </br> 选股：每个板块top5</br> 买点条件判断：</br> 1.当前股票在周K级别突破boll下轨，并且月线在中轨之上，趋势向上，同时股价在历史低位判断买点 </br> 卖出条件判断: </br> 1. 当前股价在月K级别RSI超过70","boll_rsi_v2"])
